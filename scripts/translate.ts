...
export async function TranslateQuizByNo(
	no: number,
...
export async function TranslateQuiz(
	...
	const readmePath = resolveFilePath(
		path.join(QUIZ_ROOT, quiz.path),
		"README",
		"md",
		to,
	);
	...
	console.log(
		`Translated [${quiz.no}] ${from} → ${to} | saved to ${readmePath}`,
	);
}
...
export async function translateMarkdown(
	...
	const result = (rawResult as string).replace(
		/__\s*?(\d+?)\s*?__/g,
		(_, i) => code_blocks[+i],
	);
...
export async function TranslateAllQuizes(
	from: SupportedLocale,
	to: SupportedLocale,
) {
