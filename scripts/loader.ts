...
export async function loadQuiz(dir: string): Promise<Quiz> {
	return {
		...
		path: dir,
		info: await loadLocaleVariations(
			path.join(QUIZ_ROOT, dir, "info.yml"),
			loadInfo,
		),
		readme: await loadLocaleVariations(
			path.join(QUIZ_ROOT, dir, "README.md"),
			readmeCleanUp,
		),
...
export function resolveInfo(quiz: Quiz, locale: string = defaultLocale) {
	...
	info.related =
		quiz.info[locale]?.related || quiz.info[defaultLocale]?.related || [];
