...
export function toRawREADME(quiz: Quiz, locale?: string) {
	const provider =
		"https://cdn.jsdelivr.net/gh/type-challenges/type-challenges";
...
export function toShareAnswerFull(quiz: Quiz, locale: string = defaultLocale) {
	...
	if (locale === defaultLocale)
		return `https://github.com/type-challenges/type-challenges/issues/new?labels=answer,${encodeURIComponent(
			locale,
		)}&template=0-answer.md&title=${encodeURIComponent(
			`${quiz.no} - ${info.title}`,
		)}`;
	else
		return `https://github.com/type-challenges/type-challenges/issues/new?labels=answer,${encodeURIComponent(
