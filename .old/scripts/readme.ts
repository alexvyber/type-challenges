...
export function toBadgeLink(
	...
	return `<a href="${url}" target="_blank">${toBadge(
		label,
		text,
		color,
		args,
...
function quizToBadge(
	...
	return fn(
		...
		"",
		`${quiz.no}・${
			quiz.info[locale]?.title || quiz.info[defaultLocale]?.title
		}`,
		...
		.replace(
			...
			"<!--info-footer-start--><br>" +
				...
				(Array.isArray(info.related) && info.related.length
					? `<hr><h3>${t(
							locale,
							"readme.related-challenges",
					  )}</h3>${quizNoToBadges(info.related, quizes, locale, true)}`
					: "") +
				"<!--info-footer-end-->",
		);
...
async function updateIndexREADME(quizes: Quiz[]) {
	...
	for (const locale of supportedLocales) {
		...
		const quizesByDifficulty = [...quizes].sort(
			(a, b) =>
				DifficultyRank.indexOf(a.difficulty) -
		...
		for (const quiz of quizesByDifficulty) {
			if (prev !== quiz.difficulty)
				challengesREADME += `${
					...
					quiz.difficulty,
					locale,
					quizesByDifficulty.filter((q) => q.difficulty === quiz.difficulty)
						.length,
				)}<br>`;
		...
		// by tags
		challengesREADME +=
			"<br><details><summary>By Tags</summary><br><table><tbody>";
